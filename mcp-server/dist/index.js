...
const transforms = {
    ...
    title: (text) => text.replace(/\w\S*/g, (t) => t.charAt(0).toUpperCase() + t.slice(1).toLowerCase()),
    trim: (text) => text
        .split('\n')
        .map((l) => l.trim())
        .filter((l) => l.length > 0)
...
// ── Tool: clipboard_read ──
server.tool('clipboard_read', 'Read the current system clipboard contents', {}, async () => {
    try {
...
server.tool('clipboard_write', 'Write text to the system clipboard', { text: z.string().describe('Text to copy to clipboard') }, async ({ text }) => {
    try {
...
server.tool('clipboard_history', 'Get recent clipboard history from Maccy. Returns titles, apps, categories, copy counts, and timestamps.', {
    limit: z
        .number()
        .optional()
        .default(20)
...
server.tool('clipboard_search', 'Search Maccy clipboard history by text, app name, or category (URL, Code, Email, etc.)', {
    query: z
        .string()
...
server.tool('clipboard_pinned', 'Get all pinned clipboard items from Maccy', {}, async () => {
    try {
...
server.tool('clipboard_stats', 'Get clipboard usage statistics — total items, copy counts, app breakdown, category distribution, most-copied items', {}, async () => {
    try {
        const stats = db.getStats();
        const duplicateRate = stats.totalCopies > 0
        ...
        const lines = [
            ...
            ``,
            `-- Content Types --`,
            ...Object.entries(stats.categoryBreakdown).map(([cat, count]) => `  ${cat}: ${count}`),
            ``,
            `-- Top Apps --`,
            ...Object.entries(stats.appBreakdown).map(([app, count]) => `  ${app}: ${count}`),
            ``,
            `-- Most Copied --`,
            ...stats.topCopied.map((item, i) => `  ${i + 1}. "${item.title}" (${item.count}x)`),
        ];
        return {
...
server.tool('clipboard_transform', `Apply a paste transform to text. Available transforms: ${Object.keys(transforms).join(', ')}`, {
    text: z.string().describe('Text to transform'),
    transform: z
        .string()
        .describe(`Transform name: ${Object.keys(transforms).join(', ')}`),
...
// ── Tool: clipboard_ai ──
server.tool('clipboard_ai', `AI-powered clipboard transformation using Claude. Transforms the given text according to a natural language instruction.
Examples:
  ...
  - "add TypeScript types"`, {
    ...
    instruction: z
        .string()
        .describe('What to do with the text — natural language instruction'),
    copy_result: z
        .boolean()
        .optional()
        .default(false)
...
server.tool('clipboard_context', `Build a structured context summary from clipboard history for AI agents.
...
Useful for agents that need to understand the user's current work context.`, {
    ...
    try {
        ...
        const recentItems = items.filter((item) => item.lastCopiedAt >= cutoff);
        const stats = db.getStats();
...
server.tool('clipboard_watch', 'Get clipboard items copied since a given timestamp. Useful for monitoring what the user copies during a work session.', {
    since_minutes_ago: z
        .number()
        .optional()
        .default(5)
    ...
    try {
        ...
        const formatted = recent.map((item) => {
            const ago = Math.round((Date.now() - item.lastCopiedAt.getTime()) / 1000);
